...
export default function SessionsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
      <div>
        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Mentorship Sessions</h1>
        <p className="text-gray-500 font-medium mt-1">Track your scheduled learning and teaching sessions.</p>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 space-y-4">
          ...
          <p className="text-gray-500 font-medium italic">Loading sessions...</p>
        </div>
      ) : error ? (
        <div className="bg-red-50 border border-red-100 rounded-2xl p-8 text-center">
          <p className="text-red-600 font-bold">Failed to load sessions</p>
          <p className="text-red-500 text-sm mt-1">{error}</p>
          <button onClick={loadSessions} className="mt-4 px-6 py-2 bg-red-600 text-white rounded-xl text-sm font-bold">
            Try Again
          </button>
        </div>
      ) : sessions.length === 0 ? (
        <div className="bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200 py-32 text-center">
          <div className="max-w-xs mx-auto space-y-4">
            <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto text-gray-400">
              {activeTab === 'upcoming' ? <Calendar size={32} /> : <History size={32} />}
            </div>
